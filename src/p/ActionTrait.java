...
interface ActionTrait extends Trait {
    TraitFactory.KeyChain KEY_CHAIN = new TraitFactory.KeyChain();

    ContextKey<SlideMove> KEY_SLIDEMOVE = KEY_CHAIN.newKey(ActionTrait.class, SlideMove.class);
    ContextKey<Spechits> KEY_SPECHITS = KEY_CHAIN.newKey(ActionTrait.class, Spechits.class);
    ContextKey<Movement> KEY_MOVEMENT = KEY_CHAIN.newKey(ActionTrait.class, Movement.class);

    @Override
    public default void register(TraitFactory.InsertConveyor f) {
        f.put(KEY_SLIDEMOVE, SlideMove::new);
        f.put(KEY_SPECHITS, Spechits::new);
        f.put(KEY_MOVEMENT, Movement::new);
    }
    
    AbstractLevelLoader levelLoader();
    IHeadsUp getHeadsUp();
    IDoomSystem getDoomSystem();
    IDoomStatusBar getStatusBar();
    IAutoMap<?, ?> getAutoMap();
    SceneRenderer<?, ?> sceneRenderer();

    UnifiedGameMap.Specials getSpecials();
    UnifiedGameMap.Plats getPlats();
    ...
    final class Spechits {
        line_t[] spechit = new line_t[Limits.MAXSPECIALCROSS];
        int numspechit;
