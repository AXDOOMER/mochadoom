...
public interface ActionsSight extends ActionsSpawn {
    ContextKey<Sight> KEY_SIGHT = KEY_CHAIN.newKey(ActionsSight.class, Sight.class);
    
    class Sight {
        int sightzstart; // eye z of looker
        divline_t strace = new divline_t();; // from t1 to t2
        int t2x, t2y;
    ...
    default boolean CrossSubsector(int num) {
        ...
        if (RANGECHECK) {
            if (num >= ll.numsubsectors) {
                getDoomSystem().Error("P_CrossSubsector: ss %d with numss = %d", num, ll.numsubsectors);
            }
        }
