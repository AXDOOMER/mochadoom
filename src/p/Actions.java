 ...
 * actions Playfield checks. etc. etc. so they COULD be made into a separate file
 *
 * @author admin
 *
 */
public class Actions<T, V> extends UnifiedGameMap<T, V> {
    ...
    protected void MoveCeiling(ceiling_t ceiling) {
        ...
        switch (ceiling.direction) {
            ...
            case 1:
                ...
                if (!eval(DOOM.leveltime & 7)) {
                    switch (ceiling.type) {
                        case silentCrushAndRaise:
                            break;
                        default:
                            DOOM.doomSound.StartSound(ceiling.sector.soundorg, sfxenum_t.sfx_stnmov);
                ...
                if (res == result_e.pastdest) {
                    switch (ceiling.type) {
                        ...
                        case silentCrushAndRaise:
                            DOOM.doomSound.StartSound(ceiling.sector.soundorg, sfxenum_t.sfx_pstop);
    ...
    protected void MoveFloor(floormove_t floor) {
        ...
        if (!eval(DOOM.leveltime & 7)) {
            DOOM.doomSound.StartSound(floor.sector.soundorg, sfxenum_t.sfx_stnmov);
        }
        ...
        if (res == result_e.pastdest) {
            ...
            DOOM.doomSound.StartSound(floor.sector.soundorg, sfxenum_t.sfx_pstop);
        }
    ...
    protected boolean DoDonut(line_t line) {
        ...
        rtn = false;
        while ((secnum = FindSectorFromLineTag(line, secnum)) >= 0) {
            s1 = DOOM.levelLoader.sectors[secnum];
                    ...
                    for (i = 0; i < sec.linecount; i++) {
                        if (twoSided(secnum, i)) {
                            ...
                            if (side.bottomtexture >= 0) {
                                if (DOOM.textureManager.getTextureheight(side.bottomtexture)
                                        < minsize) {
                                    minsize
                                            = DOOM.textureManager.getTextureheight(side.bottomtexture);
                                }
                            }
                ...
                if (!eval(--door.topcountdown)) {
                    switch (door.type) {
                        case blazeRaise:
                            ...
                            DOOM.doomSound.StartSound(door.sector.soundorg, sfxenum_t.sfx_bdcls);
                            break;

                        case normal:
                            door.direction = -1; // time to go back down
                            DOOM.doomSound.StartSound(door.sector.soundorg, sfxenum_t.sfx_dorcls);
                            break;

                        case close30ThenOpen:
                            door.direction = 1;
                            DOOM.doomSound.StartSound(door.sector.soundorg, sfxenum_t.sfx_doropn);
                            break;
     ...
     */
    protected boolean DoLockedDoor(line_t line,
            vldoor_e type,
            mobj_t thing) {
        player_t p;
            ...
            switch (type) {
                ...
                case blazeOpen:
                    ...
                    if (door.topheight != sec.ceilingheight) {
                        DOOM.doomSound.StartSound(door.sector.soundorg, sfxenum_t.sfx_bdopn);
                    }
                    break;
    ...
    protected void VerticalDoor(line_t line, mobj_t thing) {
        ...
        sec = DOOM.levelLoader.sides[line.sidenum[side ^ 1]].sector;
        // secnum = sec.id;

        if (sec.specialdata != null) {

            if (sec.specialdata instanceof plat_t) // [MAES]: demo sync for e1nm0646: emulates active plat_t interpreted
            // as door. TODO: add our own overflow handling class.
            {
                door = ((plat_t) sec.specialdata).asVlDoor(DOOM.levelLoader.sectors);
            } else {
        ...
        switch (line.special) {
            ...
            case 118:    // BLAZING DOOR OPEN
                DOOM.doomSound.StartSound(sec.soundorg, sfxenum_t.sfx_bdopn);
                break;

            case 1:  // NORMAL DOOR SOUND
            case 31:
                DOOM.doomSound.StartSound(sec.soundorg, sfxenum_t.sfx_doropn);
                break;
    ...
    boolean Move(mobj_t actor) {
        ...
        if (actor.movedir >= 8) {
            DOOM.doomSystem.Error("Weird actor.movedir!");
        }
    ...
    void NewChaseDir(mobj_t actor) {
        ...
        if (actor.target == null) {
            DOOM.doomSystem.Error("P_NewChaseDir: called with no target");
        }
        ...
        if (eval(DOOM.random.P_Random() & 1)) {
            for (tdir = DI_EAST;
                    tdir <= DI_SOUTHEAST;
                    tdir++) {
                if (tdir != turnaround) {
    ...
    private boolean TryWalk(mobj_t actor) {
        ...
        actor.movecount = DOOM.random.P_Random() & 15;
        return true;
    }
...
// initiate teleport sound
        DOOM.doomSound.StartSound(mo, sfxenum_t.sfx_telept);

// spawn a teleport fog at the new spot
        ss = DOOM.levelLoader.PointInSubsector(x, y);
...
// remove the old monster,
    ...
    public mobj_t SpawnMobj(@fixed_t int x, @fixed_t int y, @fixed_t int z, mobjtype_t type) {
        ...
        if (DOOM.gameskill != skill_t.sk_nightmare) {
...
// wait at least 30 seconds
        if (DOOM.leveltime - itemrespawntime[iquetail] < 30 * 35) {
            return;
        }
...
// spawn a teleport fog at the new spot
        ...
        DOOM.doomSound.StartSound(mo, sfxenum_t.sfx_itmbk);
...
// spawn it
    ...
    mobj_t SpawnMapThing(mapthing_t mthing) {
        ...
        if (mthing.type == 11) {
            if (DOOM.deathmatch_p < 10/*DM.deathmatchstarts[10]*/) {
                // memcpy (deathmatch_p, mthing, sizeof(*mthing));
                DOOM.deathmatchstarts[DOOM.deathmatch_p] = new mapthing_t(mthing);
                DOOM.deathmatch_p++;
            }
            return null;
        }
        ...
        if (mthing.type <= 4 && mthing.type > 0) // killough 2/26/98 -- fix crashes
        {
            ...
            DOOM.playerstarts[mthing.type - 1] = new mapthing_t(mthing);
            if (!DOOM.deathmatch) {
                SpawnPlayer(mthing);
            }
        ...
        if (!DOOM.netgame && eval(mthing.options & 16)) {
            return null;
        }
        ...
        if (DOOM.deathmatch && eval(mobjinfo[i].flags & MF_NOTDMATCH)) {
            return null;
        }
    ...
    protected mobj_t SpawnMissile(mobj_t source, mobj_t dest, mobjtype_t type) {
        ...
        if (th.info.seesound != null) {
            DOOM.doomSound.StartSound(th, th.info.seesound);
        }

        th.target = source;    // where it came from
        an = DOOM.sceneRenderer.PointToAngle2(source.x, source.y, dest.x, dest.y) & BITS32;

        // fuzzy player
        if (eval(dest.flags & MF_SHADOW)) {
            an += (DOOM.random.P_Random() - DOOM.random.P_Random()) << 20;
        }
    ...
    public void DamageMobj(mobj_t target, mobj_t inflictor, mobj_t source, int damage) {
        ...
        if ((player != null) && DOOM.gameskill == skill_t.sk_baby) {
        ...
        if ((inflictor != null)
            ...
            ang = DOOM.sceneRenderer.PointToAngle2(inflictor.x,
                    inflictor.y,
                    target.x,
                    target.y) & BITS32;
        ...
        if (player != null) {
            ...
            if (player == DOOM.players[DOOM.consoleplayer]) {
                DOOM.doomSystem.Tactile(40, 10, 40 + temp * 2);
            }
        }
        ...
        if ((DOOM.random.P_Random() < target.info.painchance)
    ...
    public void KillMobj(mobj_t source, mobj_t target) {
        ...
        } else if (!DOOM.netgame && ((target.flags & MF_COUNTKILL) != 0)) {
        ...
        if (target.player != null) {
            ...
            if (target.player == DOOM.players[DOOM.consoleplayer]
                    && DOOM.automapactive) {
...
//
     ...
     */
    void CrossSpecialLine(line_t line, int side, mobj_t thing) {
        //line_t line;
        boolean ok;
    ...
    // P_TeleportMove
    //
    public boolean TeleportMove(mobj_t thing, int x, /*fixed*/ int y) {
        int xl;
        int xh;
        int yl;
        int yh;
        ...
        DOOM.sceneRenderer.increaseValidCount(1); // This is r_main's ?
        numspechit = 0;

        // stomp on any things contacted
        xl = DOOM.levelLoader.getSafeBlockX(tmbbox[BOXLEFT] - DOOM.levelLoader.bmaporgx - MAXRADIUS);
        xh = DOOM.levelLoader.getSafeBlockX(tmbbox[BOXRIGHT] - DOOM.levelLoader.bmaporgx + MAXRADIUS);
        yl = DOOM.levelLoader.getSafeBlockY(tmbbox[BOXBOTTOM] - DOOM.levelLoader.bmaporgy - MAXRADIUS);
        yh = DOOM.levelLoader.getSafeBlockY(tmbbox[BOXTOP] - DOOM.levelLoader.bmaporgy + MAXRADIUS);
        ...
        DOOM.levelLoader.SetThingPosition(thing);
    ...
    public boolean CheckPosition(mobj_t thing, @fixed_t int x, @fixed_t int y) {
        ...
        // check lines
        xl = DOOM.levelLoader.getSafeBlockX(tmbbox[BOXLEFT] - DOOM.levelLoader.bmaporgx);
        xh = DOOM.levelLoader.getSafeBlockX(tmbbox[BOXRIGHT] - DOOM.levelLoader.bmaporgx);
        yl = DOOM.levelLoader.getSafeBlockY(tmbbox[BOXBOTTOM] - DOOM.levelLoader.bmaporgy);
        yh = DOOM.levelLoader.getSafeBlockY(tmbbox[BOXTOP] - DOOM.levelLoader.bmaporgy);
        ...
        if (xl <= DOOM.levelLoader.blockmapxneg) {
            xl = 0x1FF & xl;         // Broke width boundary
        }
        if (xh <= DOOM.levelLoader.blockmapxneg) {
            xh = 0x1FF & xh;    // Broke width boundary
        }
        if (yl <= DOOM.levelLoader.blockmapyneg) {
            yl = 0x1FF & yl;        // Broke height boundary
        }
        if (yh <= DOOM.levelLoader.blockmapyneg) {
    ...
    private void HitSlideLine(line_t ld) {
        ...
        lineangle = DOOM.sceneRenderer.PointToAngle2(0, 0, ld.dx, ld.dy);

        if (side == true) {
            lineangle += ANG180;
        }

        moveangle = DOOM.sceneRenderer.PointToAngle2(0, 0, tmxmove, tmymove);
            ...
            if (!TryMove(mo, ptryx, ptryy)) {
                ...
                if (mo.player != null) {   // try to slide along it
                    ...
                    if (ceilingline != null
                            ...
                            && ceilingline.backsector.ceilingpic == DOOM.textureManager.getSkyFlatNum()) {
     ...
     * @param distance int
     */
    int AimLineAttack(mobj_t t1, long angle, int distance) {
        int x2, y2;
        shootthing = t1;
    ...
    boolean UseTraverse(intercept_t in) {
        ...
        if (line.special == 0) {
            LineOpening(line);
            if (openrange <= 0) {
                DOOM.doomSound.StartSound(usething, sfxenum_t.sfx_noway);
     ...
     */
    public boolean UseSpecialLine(mobj_t thing, line_t line, boolean side) {
    ...
    void RadiusAttack(mobj_t spot, mobj_t source, int damage) {
        ...
        yl = DOOM.levelLoader.getSafeBlockY(spot.y - dist - DOOM.levelLoader.bmaporgy);
        xh = DOOM.levelLoader.getSafeBlockX(spot.x + dist - DOOM.levelLoader.bmaporgx);
        xl = DOOM.levelLoader.getSafeBlockX(spot.x - dist - DOOM.levelLoader.bmaporgx);
        bombspot = spot;
        bombsource = source;
        bombdamage = damage;
    ...
    // P_ChangeSector
    //
    boolean ChangeSector(sector_t sector, boolean crunch) {
        int x;
        int y;
    ...
    @P_MapUtl.C(P_BlockLinesIterator) public boolean BlockLinesIterator(int x, int y, Predicate<line_t> func) {
        ...
        offset = y * DOOM.levelLoader.bmapwidth + x;

        // The index contains yet another offset, but this time 
        offset = DOOM.levelLoader.blockmap[offset];

        // MAES: blockmap terminating marker is always -1
        final int validcount = DOOM.sceneRenderer.getValidCount();

        // [SYNC ISSUE]: don't skip offset+1 :-/
        for (int list = offset; (lineinblock = DOOM.levelLoader.blockmap[list]) != -1; list++) {
            ld = DOOM.levelLoader.lines[lineinblock];
    ...
    @P_MapUtl.C(P_PathTraverse) private boolean PathTraverse(int x1, int y1, int x2, int y2, int flags, Predicate<intercept_t> trav) {
        ...
        if (((x1 - DOOM.levelLoader.bmaporgx) & (MAPBLOCKSIZE - 1)) == 0) {
            x1 += FRACUNIT; // don't side exactly on a line
        }
        if (((y1 - DOOM.levelLoader.bmaporgy) & (MAPBLOCKSIZE - 1)) == 0) {
        ...
        // for prBoom+
        _x1 = (long) x1 - DOOM.levelLoader.bmaporgx;
        _y1 = (long) y1 - DOOM.levelLoader.bmaporgy;
        ...
        _x2 = (long) x2 - DOOM.levelLoader.bmaporgx;
        _y2 = (long) y2 - DOOM.levelLoader.bmaporgy;
        xt2 = (int) (_x2 >> MAPBLOCKSHIFT);
        yt2 = (int) (_y2 >> MAPBLOCKSHIFT);

        x1 -= DOOM.levelLoader.bmaporgx;
        y1 -= DOOM.levelLoader.bmaporgy;
        x2 -= DOOM.levelLoader.bmaporgx;
        y2 -= DOOM.levelLoader.bmaporgy;
    ...
    void SpawnSpecials() {
      ...
      episode = 2;
        ...
        if (DOOM.cVarManager.bool(CommandVariable.AVG) && DOOM.deathmatch) {
            SPECS.levelTimer = true;
            SPECS.levelTimeCount = 20 * 60 * 35;
        }

        if (DOOM.deathmatch) {
            DOOM.cVarManager.with(CommandVariable.TIMER, 0, (Integer i) -> {
                SPECS.levelTimer = true;
        ...
        for (int i = 0; i < DOOM.levelLoader.numlines; i++) {
            switch (DOOM.levelLoader.lines[i].special) {
                case 48:
                    ...
                    SPECS.linespeciallist[SPECS.numlinespecials] = DOOM.levelLoader.lines[i];
        ...
        switch (plat.status) {
            case up:
                ...
                if (plat.type == plattype_e.raiseAndChange
                    ...
                    if (!eval(DOOM.leveltime & 7)) {
                        DOOM.doomSound.StartSound(plat.sector.soundorg, sfxenum_t.sfx_stnmov);
                    }
                }
                ...
                if (res == result_e.crushed && (!plat.crush)) {
                    ...
                    DOOM.doomSound.StartSound(plat.sector.soundorg, sfxenum_t.sfx_pstart);
                } else {
                    if (res == result_e.pastdest) {
                        plat.count = plat.wait;
                        plat.status = plat_e.waiting;
                        DOOM.doomSound.StartSound(plat.sector.soundorg, sfxenum_t.sfx_pstop);
